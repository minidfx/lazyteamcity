defmodule LazyTeamcity.TeamcityFetcherServer do
  ...
  defp looping(),
    do:
      Process.send_after(
        __MODULE__,
        :loop,
        Application.fetch_env!(:lazy_teamcity, :fetcher_interval_seconds)
      )
  ...
  defp try_get_build(%{"id" => _} = build_json) do
    ...
    build =
      Enum.reduce(
        [
          &try_get_build_date/2,
          &try_get_branch_name/2,
          &try_get_start_date/2
        ],
        build,
        fn x, acc -> x.(acc, build_json) end
      )
