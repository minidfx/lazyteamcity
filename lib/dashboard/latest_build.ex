defmodule LazyTeamcity.Dashboard.LatestBuild do
  @behaviour LazyTeamcity.Dashboard.Components

  alias LazyTeamcity.Build
  alias LazyTeamcity.Dashboard.Helper
  alias LazyTeamcity.State

  import Ratatouille.View
  import LazyTeamcity.Dashboard.Guards, only: [full_screen: 1]

  @impl true
  def show(%State{overlay: true} = state) when full_screen(state), do: false
  def show(%State{overlay: _}), do: true

  @impl true
  def draw(%State{latest_builds: []} = model) do
    panel(title: " \uf14a Latest builds ", height: compute_latest_build_height(model)) do
      label([text(content: "No builds")])
    end
  end

  @impl true
  def draw(%State{latest_builds: builds} = model) do
    panel(title: " \uf14a Latest builds ", height: compute_latest_build_height(model)) do
      row do
        column(size: 3) do
          label([text(content: "Date")])
        end

        column(size: 1) do
          label([text(content: "Branch")])
        end

        column(size: 3) do
          label([text(content: "Name")])
        end

        column(size: 5) do
          label([text(content: "Status")])
        end
      end

      Enum.map(builds, &draw_row(model, &1))
    end
  end

  @spec compute_latest_build_height(State.t()) :: non_neg_integer()
  def compute_latest_build_height(%State{height: x}), do: div(x, 2) - 1

  @spec get_lines_available(State.t()) :: non_neg_integer()
  def get_lines_available(%State{} = model), do: compute_latest_build_height(model) - 5

  # Private

  defp draw_row(%State{} = model, %Build{build_date: date, selected: true} = build) when not is_nil(date) do
    row do
      Helper.yield_selected_column(model, 3, Helper.datetime_to_string(date))
      Helper.yield_selected_column(model, 1, Helper.trim_branch(build))
      Helper.yield_selected_column(model, 3, build.name)
      Helper.yield_selected_column(model, 5, build.status_text || "no-status")
    end
  end

  defp draw_row(%State{} = model, %Build{build_date: date} = build) when not is_nil(date) do
    row do
      Helper.yield_column(model, 3, Helper.datetime_to_string(date), Helper.to_color(build))
      Helper.yield_column(model, 1, Helper.trim_branch(build), Helper.to_color(build))
      Helper.yield_column(model, 3, build.name, Helper.to_color(build))
      Helper.yield_column(model, 5, build.status_text || "no-status", Helper.to_color(build))
    end
  end
end
